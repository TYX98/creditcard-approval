...
def index():
    if request.method == "POST":
        ...
        s1 = "The credit card approval based on decision tree is " + str(pred1)
        model2 = joblib.load('CCD_Reg')
        pred2 = model2.predict([[income, age, loan]])
        s2 = "The credit card approval based on regression is " + str(pred2)
        model3 = joblib.load('CCD_NN')
        pred3 = model3.predict([[income, age, loan]])
        s3 = "The credit card approval based on neural network is " + str(pred3)
        model4 = joblib.load('CCD_RF')
        pred4 = model4.predict([[income, age, loan]])
        s4 = "The credit card approval based on random forest is " + str(pred4)
        model5 = joblib.load('CCD_GB')
        pred5 = model5.predict([[income, age, loan]])
        s5 = "The credit card approval based on gradient boosting is " + str(pred5)
